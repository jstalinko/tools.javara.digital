...
const addFiles = (newFiles) => {
    ...
    if (pngFiles.length !== newFiles.length) {
        error.value = 'Only PNG images are supported. Other files were ignored.';
    }

    if (files.value.length + pngFiles.length > 10) {
...
const processFiles = async () => {
    ...
    try {
        const response = await fetch(route('image.png-to-webp.process'), {
            method: 'POST',
            headers: {
...
const downloadAll = () => {
    ...
    window.location.href = route('image.png-to-webp.download-zip', { id: batchId.value });
};
...
<template>
    <HomeLayout title="PNG to WebP Converter - JavaraDigital">
            ...
            <div class="z-10 w-full max-w-4xl flex flex-col gap-10">
                ...
                <div class="text-center">
                    ...
                    <h1 class="text-white text-4xl md:text-5xl font-extrabold tracking-tight mb-2">
                        PNG to <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-emerald-400">WebP</span>
                    </h1>
                    <p class="text-zinc-500 text-xs mt-1 uppercase tracking-widest font-bold">
                        Convert multiple images with PHP processing power
                    </p>
                </div>
                ...
                <div class="w-full space-y-8">
                    ...
                    >
                        <label class="flex flex-col items-center justify-center py-16 px-6 cursor-pointer rounded-[38px] border-2 border-dashed border-zinc-800 group-hover:bg-zinc-950/50 transition-all">
                            ...
                            <div class="text-center">
                                <span class="text-white text-xl font-bold block mb-2">Drop your PNGs here</span>
                                <span class="text-zinc-500 text-sm font-medium">Or click to browse files (PNG only, max 5MB)</span>
                            </div>
                            <input type="file" multiple accept="image/png" @change="onFileSelect" class="hidden" />
                        </label>
                    </div>
                    ...
                    <div v-if="files.length > 0" class="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div class="flex items-center justify-between px-2">
                             ...
                             <div v-if="results.length > 0" class="text-[10px] font-black uppercase text-emerald-400 bg-emerald-400/10 px-3 py-1 rounded-full border border-emerald-400/20">
